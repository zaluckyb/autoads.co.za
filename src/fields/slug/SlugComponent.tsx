...
export const SlugComponent: React.FC<SlugComponentProps> = ({
  ...
  // Live target value for auto-generation
  ...
  // Auto-fill slug when unlocked and empty, with debounce
  useEffect(() => {
    if (isLocked) return

    const handle = setTimeout(() => {
      const formatted = targetValue ? formatSlug(targetValue) : ''
      if ((!value || value === '') && formatted) {
        setValue(formatted)
      }
    }, 20000) // wait ~20 seconds before setting slug

    return () => clearTimeout(handle)
    // Only react when source field or lock state changes
  }, [targetValue, isLocked])
